...
class VowelMeasurement:
    ...
    def __init__(self):
        ...
        self.tracks = []         ## formant "tracks" (five sample points at 20%, 35%, 50%, 65% and 80% of the vowel)
        self.all_tracks = []     ## formant "tracks" for all possible formant settings (needed for remeasurement)
...
class VowelMean:
    ...
    def __init__(self):
        ...
        self.trackvalues = []            ## formant "tracks" (5 measurement points) from individual tokens
...
def addStyleCodes(words, tg):
    ...
    for w in words:
        ## iterate over the style tier from approximately the point where the style code for the last word was found
        for i, s in enumerate(tg[-1][i_start:]):
            ## break off style tier iteration after the end of the word
            if s.xmin() >= w.xmax:
                ## set new start interval (NOTE:  i starts counting from the previous i_start!)
                i_start += i - 2                    ## start next iteration two intervals before, just in case
                if i_start < 0:
...
def anae(v, formants, times):
    ...
    F1 = [f[0] if len(f) >= 1 else None for f in formants]  ## NOTE:  'None' elements in the two formant lists are needed
    F2 = [f[1] if len(f) >= 2 else None for f in formants]  ##        to preserve the alignment with the 'times' list!
...
def checkConfigValue(f, option, value):
    ...
    if option == 'vowelSystem':
        allowedValues = ['phila', 'Phila', 'PHILA', 'NorthAmerican', 'simplifiedARPABET']
        checkAllowedValues(f, option, value, allowedValues)
...
def faav(phone, formants, times, intensity):
    ...
    if (phone.label[:-1] in ["AY", "EY", "OW", "AW"]) or (phone.label[:-1] == "UW" and phone.cd == "73"):
        ...
        ## measure "OW" and "AW" halfway between beginning of segment and F1 maximum
...
def getFormantTracks(poles, times, xmin, xmax):
    ...
    dur = xmax - xmin
    ## get measurement points for formant tracks (20%, 35%, 50%, 65% and 80% into the vowel)
    measurement_times = [xmin + (0.2 * dur) + (0.15 * dur * i) for i in range(5)]
...
def getTimeOfF1Maximum(formants, times, beg_cutoff, end_cutoff):
    ...
    ## get F1 maximum
    F1 = [f[0] if f else 0 for f in trimmedFormants]  ## 'else' for those weird cases where there is a hole in the formant tracks...
    i = F1.index(max(F1))
...
def measureVowel(phone, word, poles, bandwidths, times, intensity, measurementPointMethod, formantPredictionMethod, padBeg, padEnd, means, covs):
    ...
    if formantPredictionMethod == 'mahalanobis':
        ...
        for j in range(4):
            ...
            ##        For "lennig", "anae" and "faav", which depend on the shape of the formant tracks, different results will be obtained for different nFormants settings.
        ...
        ## check that we actually do have a measurement (this may not be the case for gaps in the wave form)
...
def modifyIntensityCutoff(beg_cutoff, end_cutoff, phone, intensities, times):
    ...
    if end_cutoff > midpoint and midpoint > beg_cutoff:
        end_cutoff = midpoint
    ## exclude cases where the intensity maximum is at the end of the segment (because of a following vowel)
...
##            print "No F2 normalized standard deviation for vowel class %s:  value = %s, stdv = %s." % (p, m_means[p].stdvs[1], grand_stdvs[1])
        ...
        for i in range(5):
            try:
                m_means[p].trackmeans_norm.append((round(650 + 150 * (lobanov(m_means[p].trackmeans[2*i][0], grand_means[0], grand_stdvs[0])), 0),
                                                   round(150 * (m_means[p].trackmeans[2*i][1] / grand_stdvs[0]), 0)))  ## mean and stdv for F1
            except TypeError:
                m_means[p].trackmeans_norm.append(('', ''))
            try:
                m_means[p].trackmeans_norm.append((round(1700 + 420 * (lobanov(m_means[p].trackmeans[2*i+1][0], grand_means[1], grand_stdvs[1])), 0),
...
def outputMeasurements(outputFormat, measurements, m_means, speaker, outputFile, outputHeader):
    ...
    if outputFormat in ['txt', 'text', 'both']:
        ...
        for vm in measurements:
            fw.write('\t'.join([vm.phone, str(vm.stress), vm.word, str(vm.f1)]))    ## vowel (ARPABET coding), stress, word, F1
            fw.write('\t')
            if vm.f2:
        ...
        ## individual measurements
        for vm in measurements:
            fw.write('\t'.join([vm.phone, str(vm.stress), vm.word, str(vm.norm_f1), str(vm.norm_f2)]))    ## vowel (ARPABET coding), stress, word, F1, F2
            fw.write('\t')
            fw.write('\t'.join([str(vm.t), str(vm.beg), str(vm.end), str(vm.dur), vm.cd, vm.fm, vm.fp, vm.fv, vm.ps, vm.fs, vm.style, vm.glide]))
            fw.write('\t')      ## time of measurement, beginning and end of phone, duration, Plotnik environment codes, style coding, glide coding
...
def extractFormants(wavInput, tgInput, output, opts, SPATH='', PPATH=''):
    ...
    for (wavFile, tgFile, outputFile) in zip(wavFiles, tgFiles, outputFiles):
        ...
        words = getWordsAndPhones(tg, phoneset, speaker, vowelSystem)       ## (all initial vowels are counted here)
        global maxTime
        ...
        for w in words:
            ...
            if uncertain.search(w.transcription):
                ...
                #print "\t\t\t...word %s is uncertain transcription." % w.transcription
                continue
