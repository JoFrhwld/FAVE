...
#  print ','.join([ac, prec_p, foll_p, fm, fp, fv, ps, fs, trans])
  ...
  ## if different consonant inventories are used, just use the plain conversions
...
def cmu2plotnik_code(i, phones, trans, phoneset, speaker, vowelSystem):
  ...
  code = arpabet2plotnik(re.findall(r'^([A-Z]{2,2})\d?$', phones[i].label.upper())[0], trans, prec_p, foll_p, phoneset, fm, fp, fv, ps, fs)
...
def outputPlotnikFile(Plt, f):
  ...
  for vm in Plt.measurements:
    ...
    if vm.f2:                                                          ## F1, F2, F3, vowel and environmental coding, stress and duration, token
      ...
      fw.write(','.join([str(round(vm.f1, 1)), str(vm.f2), str(vm.f3), vm.code, stress + '.' + str(dur), vm.word]))  
    if vm.glide:
...
#    fw.write('+' + ','.join([str(b) for b in vm.bandwidths]) + '+')   ## list of original bandwidths as returned from LPC analysis (at point of measurement)
    fw.write(' <' + ','.join([str(round(t, 0)) if t else '' for t in vm.tracks]) + '>')
    fw.write('\r')
  ...
  for p in PLOTNIKCODES:
    ...
    fw.write(' <' + ','.join([str(round(t[0], 0)) if t[0] else '' for t in Plt.means[p].trackmeans]) + '>')
    fw.write('\r')
  fw.close()
  ...
  for vm in Plt.measurements:
    ...
    if vm.norm_f2:                                                     ## F1, F2, F3, vowel and environmental coding, stress and duration, token
      ...
      fw.write(','.join([str(round(vm.norm_f1, 1)), str(vm.norm_f2), str(vm.norm_f3), vm.code, stress + '.' + str(dur), vm.word]))      
    if vm.glide:
      fw.write(' {' + vm.glide + '}')                                  ## glide annotation, if applicable
    if vm.style:  
      fw.write(' -' + style2plotnik(vm.style, vm.word) + '-')          ## style coding, if applicable
    if vm.nFormants:
      fw.write(' /' + str(vm.nFormants) + '/')                         ## nFormants (if Mahalanobis method)
    fw.write(' ' + str(vm.t) + ' ')                                    ## measurement point
    fw.write(' <' + ','.join([str(round(t, 0)) if t else '' for t in vm.norm_tracks]) + '>')
    fw.write('\r')
  ...
  for p in PLOTNIKCODES:
    ...
    fw.write(' <' + ','.join([str(round(t[0], 0)) if t[0] else '' for t in Plt.means[p].trackmeans_norm]) + '>')
    fw.write('\r')
  fw.close()
...
##  if pc != orig_pc:
##    print "\tPhila system reassignment:  Changed class of vowel %s from %2s to %2s in word %s (%s)." % (phones[i].label, orig_pc, pc, trans, ' '.join([p.label for p in phones]))
